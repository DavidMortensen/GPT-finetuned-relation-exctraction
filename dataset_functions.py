...
class DatasetFunctions:
    ...
    def _mask_entities(tokens, entity_offsets, first_entity_replace, second_entity_replace):
        ...
        if first_entity[0] > second_entity[0]:
            tokens, first_entity, token_diff = DatasetFunctions._replace_tokens(tokens, first_entity, first_entity_replace)
            tokens, second_entity, token_diff = DatasetFunctions._replace_tokens(tokens, second_entity, second_entity_replace)
        ...
        with open(path_to_file) as f:
            for line in f.readlines():
                ...
                if masking_mode is not None:
                    example = DatasetFunctions.apply_masking_mode(example, masking_mode)
    ...
    def fetch(path_to_data, dev_size, seed, train_file='train.jsonl', test_file='test.jsonl', negative_label=None,
        ...
        if predefined_dev_set:
            ...
            sentences_train, entities_train, labels_train, ids_train = \
                DatasetFunctions._load_from_jsonl(join(path_to_data, train_file), is_test=False, masking_mode=masking_mode)
            sentences_dev, entities_dev, labels_dev, ids_dev = \
                DatasetFunctions._load_from_jsonl(join(path_to_data, dev_file), is_test=False, masking_mode=masking_mode)
        else:
        ...
        if subsampling_rate < 1.0:
            ...
            sentences_train, entities_train, labels_train, ids_train =\
                DatasetFunctions._subsample(sentences_train, entities_train, labels_train, ids_train, negative_label, subsampling_rate)
        ...
        if not skip_test_set:
            sentences_test, entities_test, labels_test, ids_test = \
                DatasetFunctions._load_from_jsonl(join(path_to_data, test_file), is_test=True, masking_mode=masking_mode)
    ...
    def encode(*splits, text_encoder, label_encoder):
        ...
        for split in splits:
            ...
            fields.append(text_encoder.encode(split[0], special_tokens=DatasetFunctions.MASKED_ENTITY_TOKENS))
            ...
            for sentence, entities in zip(split[0], split[1]):
                ...
                for start, end in entities:
                    encoded_entity.append(text_encoder.encode([sentence[start: end]], special_tokens=DatasetFunctions.MASKED_ENTITY_TOKENS)[0])
    ...
    def apply_masking_mode(example, masking_mode):
        ...
        if masking_mode == 'grammar':
            ...
            first_entity_replace, second_entity_replace = DatasetFunctions.UNK_TYPES[0], DatasetFunctions.UNK_TYPES[0]
        elif masking_mode == 'unk_w_position':
            first_entity_replace, second_entity_replace = DatasetFunctions.UNK_POS_TYPES
        else:
            raise ValueError(f"Masking mode '{masking_mode}' not supported.")

        example = example.copy()
        example['tokens'], example['entities'] = DatasetFunctions._mask_entities(
